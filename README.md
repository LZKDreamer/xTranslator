...
一个运行在 YouTube 观看页上的 Chrome Manifest V3 扩展。xTranslator 使用你自己的翻译服务和 API Key，为视频字幕、标题、简介、评论以及网页划词提供基于上下文的翻译。

> 项目仍在积极开发中。自动化测试覆盖核心逻辑；真实 YouTube 页面和真实翻译服务的组合仍建议在使用前自行验证。

## 功能

- 视频字幕翻译：支持原文、译文和双语显示模式，默认显示双语字幕。
...
设置页的字幕模式、原文/译文颜色和字号、字幕位置、划词翻译开关和上下文选项独立自动保存。字幕默认自动位于 YouTube 进度条上方；也可在播放器内上下拖动以避开视频自带文字，并可一键恢复默认样式与自动位置。API Key 变更后必须重新加载模型，未完成的翻译服务配置不会覆盖当前已保存配置。
