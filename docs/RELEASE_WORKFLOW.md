...
7. 调用 jsDelivr purge 刷新 `@latest` 更新清单，并同时刷新仍被 0.1.1/0.1.2 使用的 `@main` 兼容地址；轮询确认两个地址中的版本号和下载地址都已经对应本次发布，清单未刷新时，发布流程失败，不报告发布完成。
...
- `@latest` 是 jsDelivr 的可变版本别名，可能被 CDN 缓存。发布脚本会在推送后调用 purge，并验证 `@latest` 和旧版兼容用的 `@main` 清单内容，而不仅仅验证安装包是否返回 HTTP 200。
